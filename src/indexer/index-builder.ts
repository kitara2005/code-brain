...
// Cleanup old activity entries during build
...
if (!forceRebuild && hasFileMeta) {
  ...
  if (result) {
    ...
    console.error(`  Changed: ${result.filesChanged} | Deleted: ${result.filesDeleted} | Symbols updated: ${result.symbolsUpdated}`);
  } else {
...
/** Check if file_meta table has any data (indicates a prior build with tracking) */
function hasFileMetaData(db: DbDriver): boolean {
  try {
    const result = db.exec("SELECT COUNT(*) FROM file_meta");
...
/** Full build — clear and rebuild entire index from scratch */
function fullBuild(projectRoot: string, config: CodeBrainConfig, db: DbDriver, dbPath: string): void {
  const startTime = Date.now();

  // Clear index tables but KEEP activity_log + patterns
  clearIndex(db);
  ...
  db.run("BEGIN TRANSACTION");
  for (const mod of modules) {
    const dependedBy = modules.filter(m => m.dependencies.includes(mod.name)).map(m => m.name);
    insertMod.bind([
      mod.name, mod.path, "",
      JSON.stringify(mod.files.slice(0, 10).map(f => ({ file: f }))),
      JSON.stringify(mod.dependencies), JSON.stringify(dependedBy), "", mod.fileCount,
    ]);
    insertMod.step();
    insertMod.reset();
  }
  ...
  // Step 4: Import wiki data
  importWikiData(projectRoot, config, db);

  // Save metadata + git commit
...
/** Import wiki enrichment data into modules table */
function importWikiData(projectRoot: string, config: CodeBrainConfig, db: DbDriver): void {
  ...
  for (const file of wikiFiles) {
    ...
    if (purpose || gotchas) {
      db.run("UPDATE modules SET purpose = ?, gotchas = ? WHERE name = ?", [purpose, gotchas, name]);
      wikiCount++;
    }
  }
  db.run("COMMIT");
